...
namespace DV\RedactorBundle\Model;
...
class RedactorService extends ContainerAware
{
    ...
    public function uploadFile($type, $envName, UploadedFile $upFile)
    {
        ...
        $uploadPath = $this->container->get('kernel')->getRootDir() . $config['dir'];

        //$uploadPath = $config['dir'];

        $originalFileName = $upFile->getClientOriginalName();
        $newFileName = sha1(uniqId()) . $originalFileName ;
    ...
    {
        return $this->container->getParameter(sprintf('dv_redactor.%s', $envName));
    }
    ...
    {
        $settings = array();
        $config = $this->container->getParameter(sprintf('dv_redactor.%s', $envName));
